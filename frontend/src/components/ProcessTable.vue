      ...
      <tbody>
        ...
        <tr v-for="lotto in lotti" :key="lotto.id">
          ...
          <td class="actions-cell">
            <div class="btn-group">
              <button @click="avanza(lotto)" :disabled="lotto.stato !== 'creato' || userRole !== 'AGRICOLTORE'" class="btn-step">
                🍇 Vendemmia
              </button>

              <button @click="avanza(lotto)" :disabled="lotto.stato !== 'vendemmiato' || userRole !== 'SUPERVISORE'" class="btn-step">
                🧪 Fermentazione
              </button>

              <button @click="avanza(lotto)" :disabled="lotto.stato !== 'fermentato' || userRole !== 'SUPERVISORE'" class="btn-step">
                🏺 Affinamento
              </button>

              <button @click="avanza(lotto)" :disabled="lotto.stato !== 'affinato' || userRole !== 'CANTINIERE'" class="btn-step">
                🍾 Imbottigliamento
              </button>

              <button @click="avanza(lotto)" :disabled="lotto.stato !== 'imbottigliato' || userRole !== 'CORRIERE'" class="btn-step">
                🚚 Distribuzione
              </button>
...
const props = defineProps({
  lotti: Array,
  userRole: String, // Aggiunta prop per il controllo ruoli
  getStatusLabel: Function,
  getProgressWidth: Function,
  avanza: Function,
});

defineEmits(['elimina']); // Definiamo l'evento per l'admin
</script>

<style scoped>
/* Mantieni tutti i tuoi stili precedenti e aggiungi solo questo per il tasto elimina */
...
.process-table-container { overflow-x: auto; padding: 10px; }
.process-table { width: 100%; border-collapse: separate; border-spacing: 0 10px; }
.process-table th { text-align: center; padding: 15px; color: #666; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 1px; }
.process-table td { padding: 15px; background: white; border-top: 1px solid #eee; border-bottom: 1px solid #eee; text-align: center; }
...
.b-bianco { background: #f4d03f; }
.b-rosa { background: #db7093; }
.type-progress-col { display: flex; flex-direction: column; align-items: center; gap: 8px; }
.progress-container { width: 100px; height: 6px; background: #eee; border-radius: 10px; overflow: hidden; }
.progress-bar { height: 100%; background: #800020; transition: width 0.4s ease; }
.status-pill { padding: 6px 12px; border-radius: 6px; font-size: 0.8rem; font-weight: bold; display: inline-block; min-width: 110px; }
...
.btn-step { display: flex; align-items: center; gap: 5px; background: white; border: 1.5px solid #800020; color: #800020; padding: 8px 12px; border-radius: 8px; cursor: pointer; font-weight: 600; font-size: 0.85rem; transition: all 0.2s ease; white-space: nowrap; }
.btn-step:disabled { display: none; }
.btn-step:hover:not(:disabled) { background: #800020; color: white; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
</style>